...
class DRQN:
    def __init__(self, n_agents, dim_obs, dim_mix_state, dim_act, epi_max_len, epsilon=0.95, load=False):
        ...
        self.net = [RNNAgent(self.num_inputs, self.act_size), RNNAgent(self.num_inputs, self.act_size), RNNAgent(self.num_inputs, self.act_size)]
    ...
    def select_action(self, act_input):
        ...
        out_put_actions = []
        for role in range(self.role_type):
            self.qvalue, self.hidden_states[role] = self.net[role](act_input[role], self.hidden_states[role])
        ...
        for role in range(len(self.training_phase)):
            if role != 3:
                ...
                for i in range(self.epi_max_len):
                    ...
                    qvalue, hidden_states = self.net[role](act_input, hidden_states)
                    q_out.append(qvalue.view(self.batch_size,self.n_agents[role],-1))

                q_out = torch.stack(q_out, dim=1)

                chosen_action_qvals = torch.gather(q_out[:, :-1], dim=3, index=actions[role]).squeeze(3)
                ...
                for i in range(self.epi_max_len):
                    act_input = observations[role][:,i,:,:]
                    act_input= act_input.reshape(self.batch_size*self.n_agents[role], -1)
                    target_qvalue, target_hidden_states = self.target_net[role](act_input, target_hidden_states)
