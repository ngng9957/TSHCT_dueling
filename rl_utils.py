...
def get_state(cur_posture, prev_posture, cur_posture_opp, prev_posture_opp, cur_ball, prev_ball, field, goal, max_linear_velocity):
    ...
    for i in range(5):
        ...
        if i == 0:
            states[i] =[round(GK_defense_dis/pdd, 2), round(ball_robot_dis/pdd, 2), round(np.clip(ball_robot_velocity, -10, 10)/10, 2), round(ball_robot_th_error/math.pi, 2),
            ...
            states[i] =[round(D12_defense_dis/pdd, 2), round(ball_robot_dis/pdd, 2), round(np.clip(ball_robot_velocity, -10, 10)/10, 2), round(ball_robot_th_error/math.pi, 2),
                        round(cur_posture[i][TH]/math.pi, 2), round(robot_opp_goal/pdd, 2), round(robot_opp_goal_th/math.pi, 2), round(ball_opp_goal/pdd, 2), round(ball_opp_goal_th/math.pi, 2), round(np.clip(ball_opp_goal_velocity, -10, 10)/10, 2)]
        elif (i == 3) or (i == 4): 
            states[i] =[round(F12_attack_dis/pdd, 2), round(ball_robot_dis/pdd, 2), round(np.clip(ball_robot_velocity, -10, 10)/10, 2), round(ball_robot_th_error/math.pi, 2),
                        round(cur_posture[i][TH]/math.pi, 2), round(robot_opp_goal/pdd, 2), round(robot_opp_goal_th/math.pi, 2), round(ball_opp_goal/pdd, 2), round(ball_opp_goal_th/math.pi, 2), round(np.clip(ball_opp_goal_velocity, -10, 10)/10, 2)]
...
def get_action(robot_id, action_number, max_linear_velocity):
    ...
    GK_WHEELS = [  
            [  1.00*max_vel[0],   1.00*max_vel[0],  0.0,  0.0,  0.0], # go forwards
            [ -0.70*max_vel[0],  -0.70*max_vel[0],  0.0,  0.0,  0.0], # go backwards
            [  1.00*max_vel[0],   0.50*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[0],   1.00*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [  0.65*max_vel[0],   0.20*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [  0.20*max_vel[0],   0.65*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[0],   0.10*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [  0.10*max_vel[0],   0.50*max_vel[0],  0.0,  0.0,  0.0], # forwards turn
            [ -0.65*max_vel[0],  -0.20*max_vel[0],  0.0,  0.0,  0.0], # backwards turn
            [ -0.20*max_vel[0],  -0.65*max_vel[0],  0.0,  0.0,  0.0], # backwards turn
            [ -1.00*max_vel[0],  -0.50*max_vel[0],  0.0,  0.0,  0.0], # backwards turn
            [ -0.50*max_vel[0],  -1.00*max_vel[0],  0.0,  0.0,  0.0], # backwards turn
            [  0.23*max_vel[0],  -0.23*max_vel[0],  0.0,  0.0,  0.0], # turn
            [ -0.23*max_vel[0],   0.23*max_vel[0],  0.0,  0.0,  0.0], # turn
            [  1.00*max_vel[0],   0.85*max_vel[0],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.85*max_vel[0],   1.00*max_vel[0],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.50*max_vel[0],   0.50*max_vel[0],  5.0,  0.0,  0.0], # forwards kick
            [  0.50*max_vel[0],   0.50*max_vel[0],  8.0,  8.0,  0.0], # forwards kick
            [  0.00*max_vel[0],   0.00*max_vel[0],  8.0,  8.0,  0.0], # stop kick
            [  0.00*max_vel[0],   0.00*max_vel[0],  0.0,  0.0,  0.0], # stop
            ]

    D12_WHEELS = [  
            [  1.00*max_vel[2],   1.00*max_vel[2],  0.0,  0.0,  0.0], # go forwards
            [ -0.70*max_vel[2],  -0.70*max_vel[2],  0.0,  0.0,  0.0], # go backwards
            [  1.00*max_vel[2],   0.50*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[2],   1.00*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [  0.65*max_vel[2],   0.20*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [  0.20*max_vel[2],   0.65*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[2],   0.10*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [  0.10*max_vel[2],   0.50*max_vel[2],  0.0,  0.0,  0.0], # forwards turn
            [ -0.65*max_vel[2],  -0.20*max_vel[2],  0.0,  0.0,  0.0], # backwards turn
            [ -0.20*max_vel[2],  -0.65*max_vel[2],  0.0,  0.0,  0.0], # backwards turn
            [ -1.00*max_vel[2],  -0.50*max_vel[2],  0.0,  0.0,  0.0], # backwards turn
            [ -0.50*max_vel[2],  -1.00*max_vel[2],  0.0,  0.0,  0.0], # backwards turn
            [  0.23*max_vel[2],  -0.23*max_vel[2],  0.0,  0.0,  0.0], # turn
            [ -0.23*max_vel[2],   0.23*max_vel[2],  0.0,  0.0,  0.0], # turn
            [  1.00*max_vel[2],   0.85*max_vel[2],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.85*max_vel[2],   1.00*max_vel[2],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.50*max_vel[2],   0.50*max_vel[2],  5.0,  0.0,  0.0], # forwards kick
            [  0.50*max_vel[2],   0.50*max_vel[2],  8.0,  8.0,  0.0], # forwards kick
            [  0.00*max_vel[2],   0.00*max_vel[2],  8.0,  8.0,  0.0], # stop kick
            [  0.00*max_vel[2],   0.00*max_vel[2],  0.0,  0.0,  0.0], # stop
            ]


    F12_WHEELS = [  
            [  1.00*max_vel[4],   1.00*max_vel[4],  0.0,  0.0,  0.0], # go forwards
            [ -0.70*max_vel[4],  -0.70*max_vel[4],  0.0,  0.0,  0.0], # go backwards
            [  1.00*max_vel[4],   0.50*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[4],   1.00*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [  0.65*max_vel[4],   0.20*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [  0.20*max_vel[4],   0.65*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [  0.50*max_vel[4],   0.10*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [  0.10*max_vel[4],   0.50*max_vel[4],  0.0,  0.0,  0.0], # forwards turn
            [ -0.65*max_vel[4],  -0.20*max_vel[4],  0.0,  0.0,  0.0], # backwards turn
            [ -0.20*max_vel[4],  -0.65*max_vel[4],  0.0,  0.0,  0.0], # backwards turn
            [ -1.00*max_vel[4],  -0.50*max_vel[4],  0.0,  0.0,  0.0], # backwards turn
            [ -0.50*max_vel[4],  -1.00*max_vel[4],  0.0,  0.0,  0.0], # backwards turn
            [  0.23*max_vel[4],  -0.23*max_vel[4],  0.0,  0.0,  0.0], # turn
            [ -0.23*max_vel[4],   0.23*max_vel[4],  0.0,  0.0,  0.0], # turn
            [  1.00*max_vel[4],   0.85*max_vel[4],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.85*max_vel[4],   1.00*max_vel[4],  2.0,  2.0,  0.0], # forwards turn and kick
            [  0.50*max_vel[4],   0.50*max_vel[4],  5.0,  0.0,  0.0], # forwards kick
            [  0.50*max_vel[4],   0.50*max_vel[4],  8.0,  8.0,  0.0], # forwards kick
            [  0.00*max_vel[4],   0.00*max_vel[4],  8.0,  8.0,  0.0], # stop kick
            [  0.00*max_vel[4],   0.00*max_vel[4],  0.0,  0.0,  0.0], # stop
            ]
