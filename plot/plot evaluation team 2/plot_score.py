...
class Plot():
    def __init__(self, name):
        ...
        self.cur_num = 1
        self.num = 10
        self.value = [self.value1,self.value2,self.value3,self.value4]
    ...
    def run(self,name,fig_path):
        ...
        # for r in range(4):
            ...
            if r == 0:
                ...
                ax1.grid(True)
                ax2.grid(True)
                ax1.set_xticks([1.5, 6.5, 11.5, 16.5, 21.5])
            ...
            loc = [0,3,2,1]
            ax1.bar(range(loc[r],25,5), num_score, color=color[2*r+1],label=label[r]+' Score')
            ax1.bar(range(loc[r],25,5), num_scored, bottom=0, color =color[2*r], label=label[r]+' Concede')
...
if __name__ == '__main__':
    print_value = ['Performance versus Evaluation Team 2' ]
