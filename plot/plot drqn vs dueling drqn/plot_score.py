...
data_path1 = os.path.join(os.path.dirname(__file__), 'SCORE_Dueling_DRQN.pickle')
GK = 0
D12 = 1
F12 = 2
TEAM = 3
...
class Plot():
    def __init__(self, name):
        ...
        with open(data_path1,"rb") as fr:
            self.value.append(pickle.load(fr))
    ...
    def run(self,name,fig_path):
        ...
        for r in range(len(self.value)):
            ...
            color=['thistle','indigo','lightskyblue','b','lightgreen','green','moccasin','orange']
            if r == 0:
                ...
                label=['TSHCT-Dueling DRQN','QMIX','VDN','COMA']
            ax1.bar(range(2,35,7), num_score, color=color[2*r+1],label=label[r]+' Score', width = 0.6)
            ax1.bar(range(2,35,7), num_scored, bottom=0, color =color[2*r], label=label[r]+' Concede', width = 0.6)
...
if __name__ == '__main__':
    print_value = ['TSHCT-Dueling DRQN versus TSHCT-DRQN-200k' ]
    for value in print_value:
        fig_path = os.path.join(os.path.dirname(__file__), value+'.png')
        main = Plot(value)
